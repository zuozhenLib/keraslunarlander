...
K.set_session(sess)
class Agent:
    def __init__(self,actions,scaler,name='newLunarLander',):
        self.actions = actions
        self.name = name
        self.env = gym.make('LunarLander-v2')
        self.learningrate = 0.0015 #0.001better
    ...
    def trainmodel(self,batch):
        ...
        ob_next = batch[4]
        ob_x = self.scaler.transform(np.array([ob_current]))
        ob_next = self.scaler.transform(np.array([ob_next]))
    ...
    def getactions(self,observation):
        observation = self.scaler.transform(np.atleast_2d(observation))
        t = np.random.random()
        if t< self.epsilon:
